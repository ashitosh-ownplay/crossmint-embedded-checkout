...
export function crossmintIFrameService(props: CrossmintEmbeddedCheckoutProps) {
  ...
  const listenToEvents = (
    callback: (event: MessageEvent<CrossmintEvent>) => void
  ) => _listenToEvents(callback, CrossmintEvents);
  const listenToInternalEvents = (
    callback: (event: MessageEvent<IncomingInternalEvent>) => void
  ) => _listenToEvents(callback, IncomingInternalEvents);

  function emitInternalEvent(event: OutgoingInternalEvent) {
    const iframe = document.getElementById(
      embeddedCheckoutIFrameId
    ) as HTMLIFrameElement | null;
    if (iframe == null) {
      console.error(
        "[Crossmint] Failed to find crossmint-embedded-checkout.iframe"
      );
      return;
    }
    try {
